# set workspace
setwd('F:/Workspace_ZhaoCenliang/Github/ERSOD/ERSOD_SVR')
...
#View(data_test)
# load SVR model
load('ERSOD_SVR.RData')
# GPP estimation
result_svr <- data_test
GPP_pre.SVR <- predict(SVR, data_test[,-16])
result_svr$gpp_svr <- GPP_pre.SVR

RMSE_svr <- round(sqrt(mean((result_svr$gpp_svr - result_svr$GPP_VUT_NT)^2)),2)
sta_lm <- summary(lm(result_svr$gpp_svr ~ result_svr$GPP_VUT_NT))
R2_svr <- sta_lm$r.squared
...
scatter_plot = ggplot(data = result_svr,aes(x = GPP_VUT_NT, y = gpp_svr)) + 
  ...
  stat_poly_eq(
    ...
    family = "Times") + annotate("text", x= -Inf, y= Inf, hjust = -0.1, vjust = 4, 
                                 label=paste("RMSE =", RMSE_svr),family="Times",size = 4) + 
  theme_zg() +
  ...
  ylab(expression(paste('GPP'[ERSOD-SVR],' [','g C m'^-2,'day'^-1,']')))
